...
#pragma warning(disable : 4710) // function not inlined: don't want it to
  ...
  std::function<void(Key key, int scancode, Action action, int mods)>
      KeyPressFn = [](...) {};
...
struct WindowProps {
  ...
  WindowProps(const std::string &title = "TestWindow", uint32_t width = 1600,
...
private:
  ...
  // Also enables depth testing, gamma correction (if enabled), and debug output
  // (if enabled), Also enables face culling and which faces are culled (Back,
  // CCW)
