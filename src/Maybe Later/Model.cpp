...
Model::Model(const std::string &path, bool loadTextures,
             bool deleteAfterLoading)
    : m_EnableTextures(loadTextures), m_DeleteAfterLoading(deleteAfterLoading) {
#if QUICK_LOADING
  m_Loading =
      std::async(std::launch::async, [this, path]() { LoadModel(path); });
#else
  LoadModel(path);
#endif
}
...
std::vector<std::shared_future<LoadingTexture *>>
Model::LoadTexturesFromType(const aiMaterial *material, aiTextureType type,
  ...
  for (int i = 0; i < tCount; i++) {
    ...
    memcpy(fname, str.C_Str(), static_cast<size_t>(str.length) + 1);
...
static unsigned int TextureFromFile(const char *path,
  ...
  if (nrChannels == 1)
    ...
    throw "Texture in file " + filename + " is weird: has " +
        std::to_string(nrChannels) +
        " amound of channels, instead of the normal 1, 3, or 4";
...
std::vector<Texture> Model::LoadTexturesFromType(aiMaterial *mat,
                                                 aiTextureType type,
                                                 std::string typeName) {
...
void Model::Draw(Shader &shader, const Frustum &camFrustum,
                 const Transform &transform, bool setTextures) {
  if (!m_DoneOpenGL)
    return;
...
void Model::DrawInstanced(Shader &shader, unsigned int count,
                          const Frustum &camFrustum, const Transform &transform,
                          bool setTextures) {
  if (!m_DoneOpenGL)
    return;
...
void Model::LoadModel(const std::string &path) {
  ...
  pFlags |= aiProcess_SortByPType | aiProcess_FlipUVs |
            aiProcess_PreTransformVertices | aiProcess_GenSmoothNormals |
            aiProcess_CalcTangentSpace | aiProcess_Triangulate |
            aiProcess_GenUVCoords; // Needed for rendering
  pFlags |=
      aiProcess_GenBoundingBoxes; // let assimp make the AABB automatically
  // aiProcess_OptimizeGraph is incompatible with aiProcess_PreTransformVertices
  pFlags |= aiProcess_JoinIdenticalVertices | aiProcess_ImproveCacheLocality |
            aiProcess_OptimizeMeshes | aiProcess_JoinIdenticalVertices |
            aiProcess_RemoveRedundantMaterials; // Optimisations to do now while
  ...
  m_Directory =
      path.substr(0, path.find_last_of('/') + 1); // Want to include the '/'
...
void Model::ProcessMesh(aiMesh *mesh, const aiScene *scene) {
...
#if QUICK_LOADING
  // start futures: they can do their thing while the vertices and indices are
  // being processed
  std::vector<std::shared_future<LoadingTexture *>> m_Textures;
  std::vector<std::shared_future<LoadingTexture *>>
      maps; // just meant as an transport vehicle
#else
  std::vector<Texture> m_Textures;
  ...
  if (m_EnableTextures) {
    ...
    if (material->GetTextureCount(aiTextureType_DIFFUSE)) {
      maps = LoadTexturesFromType(material, aiTextureType_DIFFUSE,
                                  TextureType::diffuse);
      m_Textures.insert(m_Textures.end(), maps.begin(), maps.end());
    }
    // 2. specular maps
    if (material->GetTextureCount(aiTextureType_SPECULAR)) {
      maps = LoadTexturesFromType(material, aiTextureType_SPECULAR,
                                  TextureType::specular);
      m_Textures.insert(m_Textures.end(), maps.begin(), maps.end());
    }
    // 3. normal maps
    if (material->GetTextureCount(aiTextureType_HEIGHT)) {
      maps = LoadTexturesFromType(material, aiTextureType_HEIGHT,
                                  TextureType::normal);
      m_Textures.insert(m_Textures.end(), maps.begin(), maps.end());
    }
    // 4. height maps
    if (material->GetTextureCount(aiTextureType_AMBIENT)) {
      maps = LoadTexturesFromType(material, aiTextureType_AMBIENT,
                                  TextureType::height);
      m_Textures.insert(m_Textures.end(), maps.begin(), maps.end());
    }
  }
  ...
  for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
    ...
    if (m_EnableTextures && bool(mesh->mTextureCoords[0])) {
      ...
      if (mesh->mBitangents) {
        ...
        v.Bitangent =
            glm::vec3(mesh_bitangent.x, mesh_bitangent.y, mesh_bitangent.z);
      }
    } else
  ...
  // as the following list summarizes: diffuse:  diffuseN specular: specularN
  // normal:   normalN
  // height:   heightN
  ...
  glm::vec3 diffuse(diff[0], diff[1], diff[2]),
      specular(spec[0], spec[1], spec[2]);
  std::scoped_lock lg{m_MeshesMtx};

  m_MultiMesh.Add(vertices, mesh->mAABB, indices, std::move(m_Textures),
                  diffuse, specular,
                  m_EnableTextures && (m_Textures.size() > 0));
  return;
}
