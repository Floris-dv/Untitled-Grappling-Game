...
public:
  Model(const std::string &path, bool loadTextures = true,
        bool deleteAfterLoading = true);

  Model &operator=(Model &&model) noexcept;

  Model() noexcept : m_EnableTextures(false), m_DeleteAfterLoading(false) {}

  void Draw(Shader &shader, const Frustum &camFrustum,
            const Transform &transform, bool setTextures = true);
...
private:
  ...
  LoadTexturesFromType(const aiMaterial *material, aiTextureType type,
                       TextureType typeName);
#else
  std::vector<Texture> LoadTexturesFromType(aiMaterial *material,
                                            aiTextureType type,
                                            std::string typeName);
