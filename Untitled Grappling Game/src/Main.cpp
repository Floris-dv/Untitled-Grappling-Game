...
unsigned int loadCubeMap(std::array<std::string, 6> faces,
  ...
  for (int i = 0; i < 6; i++) {
    futures[i] = std::async(
        std::launch::async,
        [](std::string fname, std::string directory, TextureType type) {
          std::string filename = directory + '/' + fname;
          // if it's not heap allocated, the ret_text will delete it's data, and
          // then it's useless
...
int main() {
  ...
  std::array<glm::vec3, 4> pointLightPositions = {
      glm::vec3(0.7f, -6.8f, 5.0f), glm::vec3(2.3f, -0.5f, -3.3f),
      glm::vec3(-1.3f, 0.4f, 2.0f), glm::vec3(0.0f, 0.0f, -3.0f)};

  // This would be WAY better with #embed
  // Set up the Shaders:
  auto shader = std::make_shared<Shader>("src/Shaders/NoInstance.vert",
                                         "src/Shaders/MainFrag.frag");
  auto asteroidShader = std::make_shared<Shader>("src/Shaders/Instanced.vert",
                                                 "src/Shaders/MainFrag.frag");
  auto lightShader = std::make_shared<Shader>("src/Shaders/LightVert.vert",
                                              "src/Shaders/LightFrag.frag");
  Shader lightInstanced("src/Shaders/LightInstanced.vert",
                        "src/Shaders/LightFrag.frag");
#include "Shaders/Bloom.h"
  Shader bloomShader("src/Shaders/Bloom.comp");
  Shader crosshairShader("src/Shaders/Crosshair.vert",
                         "src/Shaders/Crosshair.frag");

  auto crosshairFuture =
      StartLoadingTexture("resources/Textures/Crosshair.png");

  auto levelMaterial = std::make_shared<Material>(
      asteroidShader, glm::vec3{0.3f, 0.35f, 1.0f} * 2.0f,
      glm::vec3{1.0f, 0.5f, 0.5f});

  auto mainMaterial = std::make_shared<Material>(
      shader, glm::vec3{0.3f, 0.35f, 1.0f}, glm::vec3{0.1f, 0.05f, 0.05f});

  auto finishMaterial = std::make_shared<Material>(
      shader, glm::vec3{1.0f, 0.35, 0.3f}, glm::vec3{0.05f, 0.05f, 0.1f});
  ...
  Object<MinimalVertex> sphere;
  {
    ...
    sphere = Object<MinimalVertex>(lightMaterial, vertices,
                                   MinimalVertex::Layout, indices);
  }

  // uniform buffer(s):
  // Matrices uniform block is at binding point 0:
  UniformBuffer matrixUBO(sizeof(glm::mat4) + sizeof(glm::vec4),
                          "Matrices"); // vec4 because of padding
  {
...

  {
    ...
    for (int i = -lineCount / 2; i <= lineCount / 2; i++) {
      ...
      model =
          glm::scale(glm::translate(glm::mat4(1.0f), {0.0f, 0.0f, (float)i}),
                     {(float)lineCount, (float)lineCount, (float)lineCount});
    }
    ...
    for (int i = -lineCount / 2; i <= lineCount / 2; i++) {
      ...
      model = glm::scale(
          glm::rotate(glm::translate(glm::mat4(1.0f), {(float)i, 0.0f, 0.0f}),
                      glm::half_pi<float>(), {0.0f, 1.0f, 0.0f}),
          {(float)lineCount, (float)lineCount, (float)lineCount});
    }
  ...
  for (unsigned int i = 0; i < 2; i++) {
    glTextureStorage2D(pingpongBuffer[i], 1, GL_RGBA16F,
    ...
    glNamedFramebufferTexture(pingpongFBO[i], GL_COLOR_ATTACHMENT0,
                              pingpongBuffer[i], 0);
  }
  ...
  for (unsigned int i = 0; i < 3; i++) {
    glTextureStorage2D(bloomRTs[i], Framebuffers::bloomMipCount, GL_RGBA16F,
                       Framebuffers::bloomTexSize.x,
                       Framebuffers::bloomTexSize.y);

    glGenerateTextureMipmap(bloomRTs[i]);
    glTextureParameteri(bloomRTs[i], GL_TEXTURE_MIN_FILTER,
...

  {
    auto callbackFn = [&editLevel, &level]() {
      ...
      if (editLevel) {
        Camera::SetCamera<EditingCamera>(
            Camera::CameraOptions{2.5f, 0.1f, 100.0f},
            Window::Get().GetAspectRatio(), glm::vec3{0.0f},
            glm::vec3{0.0f, 1.0f, 0.0f}, 90.0f, 0.0f);
      } else {
        level.UpdateInstanceVBO();
        Camera::SetCamera<GrapplingCamera>(
  ...
  Game game(std::make_unique<GrapplingCamera>(
                20.0f, 1.0f, Camera::CameraOptions{2.5f, 0.1f, 100.0f},
                Window::Get().GetAspectRatio(), glm::vec3{0.0f},
                glm::vec3{0.0f, 1.0f, 0.0f}, 90.0f, 0.0f),
            Level("Levels/Level1.dat", asteroidShader, shader), asteroidShader,
            shader, &Window::Get());
  ...
  while (!Window::Get().ShouldClose()) {
...
#if 0
    // const glm::mat4 Proj = editLevel ? glm::ortho<float>(-100.0f, 100.0f,
    // -100.0f, 100.0f, 0.1f, 200.0f) : Camera::Get().GetProjMatrix();
    ...
    // update the UBO's:
    {
      ...
      lightsUBO.SetData(
          384, 12, glm::value_ptr(Camera::Get()->Position)); // Spotlight pos
      lightsUBO.SetData(400, 12,
                        glm::value_ptr(Camera::Get()->Front)); // Spotlight dir
      ...
      if (level.UpdatePhysics(*(GrapplingCamera *)Camera::Get())) {
        endScreen = Endscreen(
            "Level", std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::duration<double>(glfwGetTime() -
                                                       level.GetStartTime())));
        Camera::Get()->Reset();
      }
    ...
    if (!editLevel) {
      ...
      if (Window::Get().GetMouseButtonDown(1)) {
        glReadPixels(Window::Get().GetWidth() / 2,
                     Window::Get().GetHeight() / 2, 1, 1, GL_DEPTH_COMPONENT,
                     GL_FLOAT, &depth);
    ...
    if (bloomToggled) {
      ...
      glBindImageTexture(0, bloomRTs[0], 0, GL_FALSE, 0, GL_WRITE_ONLY,
                         GL_RGBA16F);

      bloomShader.SetVec4("Params", threshold, threshold - knee, knee * 2,
                          .25f / knee);
      ...
      glDispatchCompute((GLuint)ceil(Framebuffers::bloomTexSize.x * 0.0625f),
                        (GLuint)ceil(Framebuffers::bloomTexSize.y * 0.0625f),
                        1);

      // Downsampling:
      for (int currentMip = 1; currentMip < Framebuffers::bloomMipCount;
           currentMip++) {
        glm::ivec2 mipSize =
            glm::vec2(Framebuffers::bloomTexSize) * (float)pow(2, -currentMip);

        // Ping
        bloomShader.SetVec2("LodAndMode", currentMip - 1.0f,
                            (float)BloomMode::DownSample);

        glBindImageTexture(0, bloomRTs[1], currentMip, GL_FALSE, 0,
                           GL_WRITE_ONLY, GL_RGBA16F);

        glBindTextureUnit(0, bloomRTs[0]);
        glDispatchCompute((GLuint)ceil(mipSize.x * 0.0625f),
                          (GLuint)ceil(mipSize.y * 0.0625f), 1);

        // Pong
        bloomShader.SetVec2("LodAndMode", (float)currentMip,
                            (float)BloomMode::DownSample);

        glBindImageTexture(0, bloomRTs[0], currentMip, GL_FALSE, 0,
                           GL_WRITE_ONLY, GL_RGBA16F);
      ...
      // First upsample
      glBindImageTexture(0, bloomRTs[2], Framebuffers::bloomMipCount - 1,
                         GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);

      bloomShader.SetVec2("LodAndMode", float(Framebuffers::bloomMipCount - 2),
                          (float)BloomMode::UpSample_First);

      glBindTextureUnit(0, bloomRTs[0]);

      glm::ivec2 currentMipSize =
          glm::vec2(Framebuffers::bloomTexSize) *
          (float)pow(2, -((int)Framebuffers::bloomMipCount - 1));

      glDispatchCompute((GLuint)glm::ceil((float)currentMipSize.x * 0.0625f),
                        (GLuint)glm::ceil((float)currentMipSize.y * 0.0625f),
                        1);

      bloomShader.SetInt("u_BloomTexture", 1);

      // Rest of the upsamples
      for (int currentMip = Framebuffers::bloomMipCount - 2; currentMip > -1;
           currentMip--) {
        currentMipSize =
            glm::vec2(Framebuffers::bloomTexSize) * (float)pow(2, -currentMip);
        glBindImageTexture(0, bloomRTs[2], currentMip, GL_FALSE, 0,
                           GL_WRITE_ONLY, GL_RGBA16F);

        bloomShader.SetVec2("LodAndMode", (float)currentMip,
                            (float)BloomMode::UpSample);
    ...
    crosshairShader.SetVec2(
        "uSize",
        {crosshairSize / Window::Get().GetAspectRatio(), crosshairSize});
